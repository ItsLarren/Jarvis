...
# Define the port variable at the top of the script
port = None
...
def speak(audio_string: object) -> object:
    engine.say(audio_string)
    engine.runAndWait()


speak('Welcome Engineer Larren')
time.sleep(1)  # Add a delay of 2 seconds (adjust as needed)
...
def respond(voice_data):
    ...
    # 7: wikipedia summary
    if there_exists(['summarise'], voice_data):
        port.write(b'u')
        port.write(b'l')
    ...
    if there_exists(['summarize'], voice_data):
        port.write(b'u')
        port.write(b'l')
    ...
    if there_exists(["combat mode"], voice_data):
        speak('uppercut')
        port.write(b'U')
        speak('Jarvis smash!')
        port.write(b's')
